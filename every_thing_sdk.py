...
def search(pattern):
    everything_dll.Everything_SetSearchW(pattern)
    ...
    print("{} Result Count: {}".format(pattern, num_results))
    ...
    for i in range(num_results):
        ...
        everything_dll.Everything_GetResultDateModified(
            i, date_modified_filetime)
        everything_dll.Everything_GetResultSize(i, file_size)
        if file_size.value > 10*1024*1024:  # larger than 10M
            ret_list.append((ctypes.wstring_at(filename), get_time(
                date_modified_filetime), file_size.value))
    return ret_list
